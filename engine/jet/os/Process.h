...
// string. or iterate over lines and throw away the string (more rarely).
// redirecting output to a file can always be done by shlaunch("... >
// outfile") so no special func is provided.
typedef enum {
  JET_PIPE_NONE = 0,
  JET_PIPE_READ = 1,
  JET_PIPE_WRITE = 2,
...
monostatic int test__jet_os_Process(void) {
  ...
  Pipe p = Pipe_new(cmds, JET_PIPE_WRITE);
  Pipe_write(p, "int main() {return 58;}", 23);
  Pipe_close(&p);
  Process_awaitAll();
  return EXIT_SUCCESS;
}
