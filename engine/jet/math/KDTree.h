...
static void addPoint(KDTreeNode* node, KDTreePoint* point) {
  ...
  // ith-coordinate of the node's threshold. You need to check all
  // dimensions and find out how the comparison goes (individually). In a
  // simple binary tree this is like deciding whether the new value should
  // go to the left or to the right child, but to avoid branching here it
  // goes to child[0] or child[1], and the 0 or 1 comes from the comparison
  // result. node->child is actually effectively node->child[][][] ... i.e.
  // as many []s as the dimensionality. However writing it explicitly as a
  // multi-dim array makes things difficult for a truly generic version, so
  // I'm going to write it as a normal array and calculate the overall
  // (flat) index. this N-dimensional access of the form arr[l1][l2][l3] is
  // effectively l1*4 + l2*2 + l3 ... and so on for higher dimensions, where
  // l's are either 0 or 1. so it can be reduced to bitwise ops: the *s can
  // be done with << and adding them together is a |.
...
// always init  with this func with a known number of levels and a bounding
// box this way you have a node hierarchy ready and your add calls will not
// thrash adding subnodes and moving points down over and over. KDTreeNode*
// init(int levels, KDTreePoint* boundBox[2]) { }
...
static KDTreePoint* getNearestPoint(KDTreeNode* node, KDTreePoint* point) {
  ...
  // a point is always returned. you asked for the near*est* point, and
  // there is always one, regardless of how near or far it is exactly. THATS
  // NOT TRUE!!!!! MAYBE: pass mindist back to the caller to save a repeated
...
// already. This is because you need 1 node above the current level in order
// to compute the threshold for the new nodes to be added at a new level.
...
static KDTreeNode* init(KDTreePoint boundBox[2], int sizeGuess) {
  ...
  // here's a bad trick: I'm going to set all children of the parent to
  // root. so if you do send in a point for add or get that happens to be
  // outside the bounding box, it won't crash but it will pass on to the
  // midpoint of the BB and from then on it will be placed / looked up as
  // usual. for_to(j, POW2(DIMS)) parent->child[j] = root;
