...
// needs a func growToExactly where it does not check roundUp32. Vectors may
// be used for push pop but often they will be init'd and used with a known
// size. Also need a Vector_clone func -> no call concatArray on an empty
// new Vector.
...
#define Vector__mutopv(vec, op, vec2)                                      \
  for (Real64* _v = (vec)->ref, *_ve = _v + (vec)->used,                   \
               *_v2 = (vec2)->ref;                                         \
       _v < _ve; _v++, _v2++)                                              \
    *_v op* _v2;
...
/// Perform a binop `op` (+, -, *, /) on scalar `num` and vector `vec` and
/// store result in vector `out`. `out` must be preallocated by the caller
/// and of the same size as `vec`. Inplaceable (`out` can alias `vec`).
...
// the functions are wriiten (by the user) as if vec & out do not alias.
// they are read by the compiler as if vec & out DO alias and then it checks
// if array deps prevent inplacing. if so then out must be same-sized as vec
// but NEED NOT be cloned! it is only reading from vec.of course if there
...
/// Use trapezoidal rule to integrate dy. If you want to integrate over a
/// grid dx, first do dy*dx (elemwise multiply) and then send that into
/// integrate(...).
...
/// Branch-free max and min, if you have really unpredictable data. Test
/// this at some point... yeah it has really horrible performance at -O0,
/// and at -O3 it has the same performance as branching
