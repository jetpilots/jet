...
static bool isnum(TypeTypes tyty) { return tyty >= TYInt8; }

static const char* name(TypeTypes tyty) {
    switch (tyty) {
    case TYUnresolved:
        return NULL;
    case TYNilType:
...
// these are DEFAULTS
static const char* format(TypeTypes tyty, bool quoted) {
    switch (tyty) {
    case TYUnresolved:
    case TYNoType:
    case TYNilType:
...
static unsigned int size(TypeTypes tyty) {
    switch (tyty) {
    case TYUnresolved:
    case TYNoType:
    case TYNilType:
...
// (e.g. func args)
static TypeTypes byName(const char* spec) {
...
static const char* nativeName(CollectionTypes coty) {
    switch (coty) {
    case CTYNone:
        return "";
    case CTYArray:
