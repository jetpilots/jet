...
///////////////////////////////////////////////////////////////////////////
void emit(ASTImport& import, int level) {
...
///////////////////////////////////////////////////////////////////////////
void undefc(ASTImport& import) {
    // if (import.alias) printf("#undef %s\n", import.alias);
}

///////////////////////////////////////////////////////////////////////////
void emit(ASTTypeSpec& typeSpec, int level, bool isconst) {
...
void emit(ASTExpr* expr, int level);

///////////////////////////////////////////////////////////////////////////
void emit(ASTVar& var, int level, bool isconst) {
    // for C the variables go at the top of the block, without init
    printf("%.*s", level, spaces);
    // if (var.typeSpec)
    emit(var.typeSpec, level + STEP, isconst);
    printf(" %s", var.name);
}
...
///////////////////////////////////////////////////////////////////////////
void unmarkVisited(ASTExpr& expr) {
    switch (expr.kind) {
...
void genPrintVars(ASTExpr& expr, int level) {
    assert(expr);
    ...
    case tkVarAssign:
        ...
        printf("%.*sprintf(\"    %s = %s\\n\", %s);\n", level, spaces,
            expr.var.name, TypeType_format(expr.typeType, true), expr.var.name);
...
ASTExpr* findPromotionCandidate(ASTExpr& expr) {
    // assert(expr);
    ASTExpr* ret;
    ...
    case tkFunctionCallResolved:
        ...
        if (expr.left && (ret = findPromotionCandidate(*expr.left)))
            return ret;
    ...
    default:
        if (expr.prec) {
            if ((ret = findPromotionCandidate(*expr.right))) return ret;
            if (!expr.unary)
                if ((ret = findPromotionCandidate(*expr.left))) return ret;
        }
    }
...
///////////////////////////////////////////////////////////////////////////
bool isCtrlExpr(ASTExpr& expr) {
    return expr.is(tkKeyword_if) //
        || expr.is(tkKeyword_for) //
        || expr.is(tkKeyword_while) //
        || expr.is(tkKeyword_else);
}

bool isLiteralExpr(ASTExpr& expr) { return false; }
bool isComparatorExpr(ASTExpr& expr) { return false; }

///////////////////////////////////////////////////////////////////////////
void lowerElementalOps(ASTScope& scope) {
    for (ASTExpr& stmt : scope.stmts) {

        if (isCtrlExpr(stmt) && stmt.body) lowerElementalOps(*stmt.body);
...
///////////////////////////////////////////////////////////////////////////
void promoteCandidates(ASTScope* scope) {
    int tmpCount = 0;
    ASTExpr* pc = NULL;
    ...
    for (ASTExpr& stmt : scope.stmts) {
        ...
        if (isCtrlExpr(stmt) && stmt.body) promoteCandidates(stmt.body);
    ...
    startloop:
        ...
        *pcClone = *pc;
        ...
        if (!pcClone->left)
            ...
            while (argn->is(tkOpComma) && argn->right->is(tkOpComma))
                argn = argn->right;
            ASTExpr* com = NEW(ASTExpr);
            // TODO: really should have an astexpr ctor
            com->prec = TokenKind_getPrecedence(tkOpComma);
...
///////////////////////////////////////////////////////////////////////////
void emit(ASTScope* scope, int level) {
    for (ASTVar& local ::scope.locals)
        if (local.used) {
            emit(local, level, false);
            puts(";");
        } // these will be declared at top and defined within the expr list
    for (ASTExpr& stmt : scope.stmts) {
        if (stmt.is(tkLineComment)) continue;

        if (genLineNumbers) printf("#line %d\n", stmt.line);
        // if (genCoverage) printf("    _cov_[%d]++;\n", stmt.line - 1);
        // if (genLineProfile) {
        //     printf("    _lprof_tmp_ = getticks();\n");
        //     printf("    _lprof_[%d] += (_lprof_tmp_-_lprof_l)/100;\n",
        //         stmt.line - 1);
        //     printf("    _lprof_l;\n");
        // }

        // You need to know if the emit will in fact generate something.
...
///////////////////////////////////////////////////////////////////////////
void genJsonReader(ASTType* type) { }

const char* functionEntryStuff_UNESCAPED
    = "    STACKDEPTH_UP; DO_STACK_CHECK;\n";

//   const char* functionEntryStuff_UNESCAPED__old
//     = "    STACKDEPTH_UP\n"
//       "#ifndef NOSTACKCHECK\n"
//       //   "    // printf(\"%8lu %8lu\\n\",_scUsage_, _scSize_);\n"
...
//   const char* functionExitStuff_UNESCAPED_old
// = "    return DEFAULT_VALUE;\n"
//   "error:\n"
//   "#ifdef DEBUG\n"
//   "    eprintf(\"error: %s\\n\",_err_);\n"
...
///////////////////////////////////////////////////////////////////////////
  ...
  ///////////////////////////////////////////////////////////////////////////
  void emit(ASTType* type, int level) {
      ...
      for (ASTVar& var : type->body->locals) // if (var.used)
      ...
      printf("#define DEFAULT_VALUE NULL\n"
             "mono  %s %s_new_(IFDEBUG(const char* callsite_)) {\n"
      ...
      printf(
          "mono  void %s_print__(%s self, const char* name) {\n    "
          "printf(\"<%s "
  ...
  ///////////////////////////////////////////////////////////////////////////
  void genh(ASTType* type, int level) {
      if (!type->body || !type->analysed || type->isDeclare) return;
  ...
  void genh(ASTType* type, int level) {
      ...
      const char* datType = ex1->is(tkOpAssign) ? typeName(ex1->right) : NULL;
      if (datType)
          printf("mono  %s %s__data[%d];\n", datType, name,
              PtrList_count(type->body->locals));
      printf("mono  const char* %s__fullnames[] ={\n", name);
      for (ASTVar& var : type->body->locals)
          printf("    \"%s.%s\",\n", name, var.name);
      puts("};");
      printf("mono  const char* %s__names[] ={\n", name);
      for (ASTVar& var : type->body->locals) printf("    \".%s\",\n", var.name);
      puts("};");
      // printf("mono  const char* %s__names[%d];\n", name,
      //     PtrList_count(type->body->locals));

      printf("mono  void %s__init() {\n", name);
      // for (ASTVar&  var : type->body->locals) {
  ...
  ///////////////////////////////////////////////////////////////////////////
  void emit(ASTFunc& func, int level) {
  ...
  ///////////////////////////////////////////////////////////////////////////
  void genh(ASTFunc& func, int level) {
  ...
  ///////////////////////////////////////////////////////////////////////////
  void genh(ASTVar& var, int level) {
      // if (! func->body or not func->analysed) return;
      // if (!func->isExported) printf("  ");
      // if (var.typeSpec) {
      if (!var.init) return;

      emit(var.typeSpec, level, false);
      // }
...
//     }
  ...
  ////////////////////////////////////////////////////
  void emit(ASTTest* test) // TODO: should tests not return BOOL?
  {
      if (!test->body) return;
      printf("\n  void test_%s() {\n", test->name);
      emit(test->body, STEP);
  ...
  void tkSubscriptResolved(ASTExpr* expr, int level) {
      ...
      case tkNumber: // indexing with a single number, can be a -ve number
          printf("Array_get_%s(%s, %s)", cname(expr.var.typeSpec), name,
              index->string);
          break;

      case tkString:
      case tkRawString: // indexing with single string or regex
          printf("Dict_get_CString_%s(%s, %s)", cname(expr.var.typeSpec), name,
              index->string);
          break;
      ...
      case tkOpColon:
          // a single range.
          printf("Array_getSlice_%s(%s, ", name(expr.var.typeSpec), name);
          emit(index, 0);
          printf(")");
          break;
      ...
      case tkKeyword_not:
          ...
          //    stage.
          printf("Array_copy_filter_%s(%s, ", name(expr.var.typeSpec), name);
          emit(index, 0);
          printf(")");
          break;
  ...
  void tkFunctionCallResolved(ASTExpr* expr, int level) {
      char* tmp = expr.func->selector;

      ASTExpr* arg1 = expr.left;
      const char* tmpc = "";
      if (arg1) {
          if (arg1->is(tkOpComma)) arg1 = arg1->left;
  ...
  void lineupmultilinestring(ASTExpr* expr, int indent) {
      return;
  ...
  void tkString(ASTExpr* expr, int level) {
          ...
          while (*pos) {
              ...
              *pos++ = 0;
              ...
              if (e) {
                  ...
                  assert(e->is(tkIdentifierResolved));
                  assert(e->var);
                  printf("%s", format(e->var.typeSpec.typeType, false));
                  last = pos;
                  e = ((p = p->next)) ? p->item : NULL;
              }
          }
          printf("\"");
          for (ASTExpr& e : expr.vars) {
              printf(", ");
              emit(e, 0);
          }
          printf(")");
  ...
  /// `_Complex_I * 1`.
  void emit_tkNumber(ASTExpr* expr, int level) {
  ...
  void emit_tkCheck(ASTExpr* expr, int level) {
  ...
  void emit(ASTExpr* expr, int level) {
      ...
      case tkFunctionCallResolved:
          tkFunctionCallResolved(expr, level);
          break;
      ...
      case tkSubscriptResolved:
          tkSubscriptResolved(expr, level);
          break;
      ...
      case tkOpModEq:
          ...
          case tkSubscriptResolved:
              ...
              case tkOpColon:
                  printf("%s_setSlice(%s, ", typeName(expr.left),
                      expr.left->var.name);
      ...
      case tkKeyword_match: {
          ...
          printf("{%s __match_cond = ", cond.typeName());
          emit(cond, 0);
          if (cond.isNumeric() || cond.isEnum())
              puts("; switch (__match_cond) {");
          else
              puts("; { if (0) {}"); // the cases will add 'else if's
      ...
      case tkKeyword_case: {
          ...
          ASTExpr& cond = expr.left;
          if (cond.is(tkOpComma)) {
              if (cond.isNumeric() or cond.isEnum()) {
      ...
      case tkKeyword_if:
          ...
          //    case tkKeyword_else:
      case tkKeyword_while:
          if (expr.is(tkKeyword_for))
              printf("FOR(");
          else
              printf("%s (", TokenKind_repr(expr.kind, true));
          if (expr.is(tkKeyword_for)) expr.left->kind = tkOpComma;
          if (expr.left) emit(expr.left, 0);
          if (expr.is(tkKeyword_for)) expr.left->kind = tkOpAssign;
          puts(") {");
          if (expr.body) emit(expr.body, level + STEP);
          printf("%.*s}", level, spaces);
          break;
      ...
      case tkPeriod:
          ...
          if (expr.left->typeType == TYObject
              && getObjectType(expr.left)->isEnum)
              printf("_");
          else
      ...
      case tkKeyword_in:
          ...
          case tkArrayOpen:
              if (expr.right->right->is(tkOpColon))
      ...
      case tkOpGT:
      case tkOpLT:
          if ((expr.is(tkOpLE) || expr.is(tkOpLT))
              && (expr.left->is(tkOpLE) | expr.left->is(tkOpLT))) {
      ...
      default:
          ...
          if (expr.is(tkArrayOpen)) putc('}', stdout);
      }
  }
  ...
  //     "\"r\" );"
  //     "}\n"
  //     "  void lineprofile_begin() {_lprof_l=getticks();}\n"
  // };
  ///////////////////////////////////////////////////////////////////////////
  // TODO: why do you need to pass level here?

  void genTypeInfoDecls(ASTType* type);
  void genTypeInfoDefs(ASTType* type);
  void genNameAccessors(ASTType* type);
  // TODO: this should be in ASTModule open/close
  void emit_open(Compiler& com) {
  ...
  void emit_close(Compiler& com) {
      printf("#undef THISMODULE\n");
      printf("#undef THISFILE\n");
      printf("#endif // HAVE_%s\n", com->capsMangledName);
  }

  void emit(ASTModule& module) {
      // puts("");
      for (ASTImport& import : module.imports) emit(import, 0);
      ...
      for (ASTType& type : module.enums) {
          if (type->body && type->analysed) {
              genh(type, 0);
              // genTypeInfoDecls(type);
          }
      }

      for (ASTType& type : module.types) {
          if (type->body && type->analysed) {
              genh(type, 0);
              genTypeInfoDecls(type);
          }
      ...
      for (ASTFunc& func : module.funcs) {
          if (func->body && func->analysed) { genh(func, 0); }
      }

      for (ASTType& type : module.types) {
          if (type->body && type->analysed) {
              // for (ASTExpr&  expr : type->body->stmts)
              //     prepareInterp(expr, type->body);
              // ^ MOVE THIS INTO emit
              emit(type, 0);
      ...
      for (ASTFunc& func : module.funcs) {
          if (func->body && func->analysed) {
              // for (ASTExpr&  expr : func->body->stmts) {
              //     prepareInterp(parser,expr, func->body);
              // }
              // ^ MOVE THIS INTO emit
              emit(func, 0);
          }
      }
      for (ASTImport& import : module.imports) undefc(import);

      // puts(coverageFunc[genCoverage]);
      // puts(lineProfileFunc[genLineProfile]);
  }

  void genTests(ASTModule* module) {
      emit(module);
      for (ASTTest& test : module.tests) emit(test);
      // generate a func that main will call
      printf("\nvoid tests_run_%s() {\n", module.name);
      for (ASTTest& test : module.tests) printf("    test_%s();\n", test->name);
      puts("}");
  }
      ...
      for (ASTVar& var : type->body->locals) // if (var.used) //
          if (var.typeSpec.typeType >= TYBool
              ...
              printf("    if (CString_equals(name, \"%s\"))  {self->%s = "
                     "*(%s*) "
                     "&value;return;}\n",
                  var.name, var.name, cname(var.typeSpec));
      printf("}\n");
  }
