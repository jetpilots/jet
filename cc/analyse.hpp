...
static void setStmtFuncTypeInfo(Compiler& com, ASTFunc& func) {
    ...
    if (!func.returnSpec.typeType)
        ...
        errorTypeMismatchBinOp(parser, stmt);
}

// TODO make sempassModule . same as analyseModule now
static void analyseType(Compiler& com, ASTType& type, ASTModule& mod);
static void analyse(Compiler& com, ASTFunc& func, ASTModule& mod);

///////////////////////////////////////////////////////////////////////////
static void analyseDictLiteral(Compiler& com, ASTExpr& expr, ASTModule& mod) {
    // check that
...
static void prepareInterp(Compiler& com, ASTExpr& expr, ASTScope& scope) {
    ...
    while (*pos) {
        ...
        *varend = 0;
        ASTVar& var = getVar(scope, varname);
        ...
        *varend = endchar;
        ...
        if (!wasbracket)
            while (endchar == '.') {
                ...
                *varend = 0;
                ...
                var = getVar(type, varname);
                if (!var) {
                    exdot = NULL; // reset it if was set along the chain
                    errorUnrecognizedMember(parser, type,
        ...
        if (exdot) exprvars = append(exprvars, exdot);
        pos = varend;
    }
}
//     break;
...
//     if (expr.left) prepareInterp(expr.left, scope);
//     break;
// case tkVarAssign:
//     prepareInterp(expr.var.init, scope);
//     break;
// case tkKeyword_for:
// case tkKeyword_if:
// case tkKeyword_else:
...
//     prepareInterp(expr.left, scope);
//     for (ASTExpr& subexpr, expr.body.stmts)
//         prepareInterp(subexpr, expr.body);
//     break;
// default:
//     if (expr.prec) {
//         if (!expr.unary) prepareInterp(expr.left, scope);
//         if (expr.right) prepareInterp(expr.right, scope);
//     } else {
...
static void setEnumBase(
    Compiler& com, ASTExpr& expr, ASTTypeSpec& spec, ASTModule& mod) {
    switch (expr.kind) {
    case tkPeriod:
        if (expr.left) return;
    ...
    case tkOpEQ:
    case tkOpNE:
        setEnumBase(parser, expr.left, spec, mod);
        fallthrough;
    case tkOpAssign:
    case tkArrayOpen:
        setEnumBase(parser, expr.right, spec, mod);
    default:;
    }
}

///////////////////////////////////////////////////////////////////////////
static void analyseExpr(Compiler& com, ASTExpr& expr, ASTScope& scope,
    ...
    case tkFunctionCallResolved: {
        ...
        if (countCommaList(expr.left) != expr.func.argCount)
            errorArgsCountMismatch(parser, expr);
        ...
        for (ASTVar& arg : expr.func.args) {
            ...
            if (cArg.typeType != arg.typeSpec.typeType)
                errorArgTypeMismatch(parser, cArg, arg);
    ...
    case tkFunctionCall: {
        ...
        if (expr.left) strarglabels(expr.left, bufp, 128 - ((int)(bufp - buf)));

        ASTFunc& found = getFunc(mod, buf);
        if (found) {
            expr.kind = tkFunctionCallResolved;
            expr.func = found;
            analyse(parser, found, mod);
            analyseExpr(parser, expr, scope, mod, ownerFunc, inFuncArgs);
            return;
        }
        if (!strncmp(buf, "Void_", 5))
            errorCallingFuncWithVoid(parser, expr, arg1);
        else {
            errorUnrecognizedFunc(parser, expr, buf);
                ...
                for (ASTFunc &func, mod.funcs) {
                    if (!strcasecmp(expr.string, func.name)) {
                        eprintf("\e[36;1minfo:\e[0;2m   not viable: %s with %d "
                            ...
                            func.prettySelector, func.argCount, com.filename,
                            func.line);
                        sugg++;
                    }
                    if (!func.intrinsic && strcasecmp(expr.string, func.name)
                        ...
                        eprintf("\e[36;1minfo:\e[0m did you mean: "
                                "\e[34m%s\e[0m (%s at "
                                "./%s:%d)\n",
                            func.name, func.prettySelector, com.filename,
                            func.line);
                        sugg++;
                    }
                }
    ...
    case tkVarAssign: {
        ...
        if (!init) {
            ...
            if (typeSpec.typeType == TYObject && typeSpec.type.isEnum)
                setEnumBase(parser, init, typeSpec, mod);
            ...
            if (typeSpec.dims == 0) {
                ...
                typeSpec.dims = init.collectionType == CTYTensor ? init.dims
                    : init.collectionType == CTYArray            ? 1
                                                                 : 0;
            } else if (typeSpec.dims != 1 && init.collectionType == CTYArray) {
                errorInitDimsMismatch(parser, expr, 1);
                expr.typeType = TYErrorType;
            } else if (typeSpec.dims != 2 && init.collectionType == CTYTensor) {
                errorInitDimsMismatch(parser, expr, 2);
                expr.typeType = TYErrorType;

            } else if (typeSpec.dims != 0 && init.collectionType == CTYNone) {
                errorInitDimsMismatch(parser, expr, 0);
                expr.typeType = TYErrorType;
            }
        }
    } break;
    ...
    case tkKeyword_match: {
        ...
        if (cond) {
            ...
            ASTTypeSpec& tsp = getObjectTypeSpec(cond);
            if (expr.body && tsp && tsp.type
                && tsp.type.isEnum) { // left.typeType == TYObject&&.isEnum) {
                for (ASTExpr &cas, expr.body.stmts)
                    if (cas.left) setEnumBase(parser, cas.left, tsp, mod);
            }
        }

        for (ASTExpr &stmt, expr.body.stmts) {
            ...
            if (cond && stmt.kind == tkKeyword_case && stmt.left
                && (stmt.left.typeType != cond.typeType
                    || (cond.typeType == TYObject
                        && getTypeOrEnum(stmt.left) != getTypeOrEnum(cond))))
                errorTypeMismatch(parser, cond, stmt.left);
        }
    } break;
    case tkKeyword_else:
    case tkKeyword_if:
    ...
    case tkSubscriptResolved: {
        ...
        int nhave = countCommaList(expr.left);
        if (nhave != expr.var.typeSpec.dims)
            errorIndexDimsMismatch(parser, expr, nhave);
    }
        // fallthru
    case tkSubscript:
        if (expr.left)
    ...
    case tkRawString:
        ...
        prepareInterp(parser, expr, scope);
        break;
    ...
    case tkPeriod: {
        ...
        if (!expr.left) {
            errorNoEnumInferred(parser, expr.right);
            break;
        }
        ...
        if (member.kind == tkPeriod) {
            ...
            if (member.kind != tkIdentifier) {
                errorUnexpectedExpr(parser, member);
                break;
            }
        }
        ...
        if (member.kind != tkIdentifier && member.left)
            analyseExpr(parser, member.left, scope, mod, ownerFunc, inFuncArgs);
    ...
    default:
        if (expr.prec) {
            ...
            if (ISIN(5, expr.kind, tkKeyword_in, tkKeyword_notin, tkOpAssign,
                    tkOpEQ, tkOpNE)) {
                ASTTypeSpec& spec = getObjectTypeSpec(expr.left);
            ...
            if (expr.right)
                expr.elemental = expr.right.elemental || expr.kind == tkOpColon;
            ...
            if (!expr.unary && expr.left) {
                ...
                if (expr.dims != expr.left->dims
                    ...
                    if (expr.left->dims != 0 && expr.right.dims != 0) {
                        errorBinOpDimsMismatch(parser, expr);
                        ...
                        && expr.left->dims == 1 && expr.left->kind == tkOpColon
                        && expr.right.dims == 0) {
                        // eprintf("ok `:` dims@ %d %d %d %d\n", expr.line,
                        //     expr.col, expr.left->dims, expr.right.dims);
                        // expr.dims = 1;
            ...
            if (!expr.unary && expr.left
                ...
                if (leftType == TYBool
                    ...
                    // an error.
                    errorTypeMismatchBinOp(parser, expr);
                    expr.typeType = TYErrorType;
                    ...
                    // TODO: allow enum +
                    errorInvalidTypeForOp(parser, expr);
                }
...
static void analyseType(Compiler& com, ASTType& type, ASTModule& mod) {
    ...
    // eprintf(
    //     "analyseExpr: %s at ./%s:%d\n", type.name, com.filename,
    //     type.line);
    if (type.super) {
        resolveTypeSpec(parser, type.super, mod);
        if (type.super.type == type) errorTypeInheritsSelf(parser, type);
    }
    // TODO: this should be replaced by a dict query
    for (ASTType& type2 : mod.types) {
        if (type2 == type) break;
        if (!strcasecmp(type.name, type2.name))
            errorDuplicateType(parser, type, type2);
    }
...
static void hashExprs(/* Compiler& com,  */ ASTFunc& func);

///////////////////////////////////////////////////////////////////////////
static void analyse(Compiler& com, ASTFunc& func, ASTModule& mod) {
    if (func.analysed) return;
    // eprintf("analyseExpr: %s at ./%s:%d\n", func.selector,
    // com.filename, func.line);
    ...
    for (ASTType& type : mod.types) {
        if (!strcasecmp(func.name, type.name)) {
            if (func.returnSpec && !(func.isStmt || func.isDefCtor))
                errorCtorHasType(parser, func, type);
            if (!func.returnSpec) {
            ...
            // if (func.isStmt)
            //     errorCtorHasType(this, func, type);
            if (!isupper(*func.name)) warnCtorCase(parser, func);
    ...
    if (!func.isDefCtor && !isCtor && isupper(*func.name))
        errorUnrecognizedCtor(parser, func);
    ...
    if (!com.issues.errCount && com.mode != PMLint) {

        // do (try) CSE
        hashExprs(/* parser, */ func);

        // Handle elemental operations like arr[4:50] = mx[14:60] + 3
        lowerElementalOps(func.body);
        ...
        promoteCandidates(func.body);
    }
}

static void analyseTest(Compiler& com, ASTTest& test, ASTModule& mod) {
    if (!test.body) return;

    // Check for duplicate test names and report errors.
    // TODO: this should be replaced by a dict query
    for (ASTTest& test2 : mod.tests) {
        if (test == test2) break;
        if (!strcasecmp(test.name, test2.name))
            errorDuplicateTest(parser, test, test2);
    }

    // Check unused variables in the function and report warnings.
    checkUnusedVars(parser, test);

    // Run the statement-level semantic pass on the function body.
    for (ASTExpr &stmt, test.body.stmts)
        analyseExpr(parser, stmt, test.body, mod, NULL, false);

    // Do optimisations or ANY lowering only if there are no errors
    if (!com.issues.errCount && com.mode != PMLint) {
        lowerElementalOps(test.body);
        promoteCandidates(test.body);
    }
}
