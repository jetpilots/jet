...
void handle_directory_request(int out_fd, int dir_fd, char* filename) {
  ...
  writen(out_fd, buf, cstr_len(buf));
...
static const char* get_mime_type(char* filename) {
  ...
  if (dot) { // strrchar Locate last occurrence of character in string
    ...
    while (map->extension) {
      if (strcmp(map->extension, dot) == 0) { return map->mime_type; }
      map++;
    }
  }
...
void client_error(int fd, int status, char* msg, char* longmsg) {
  ...
  sprintf(
    buf + cstr_len(buf), "Content-length: %lu\r\n\r\n", cstr_len(longmsg));
  sprintf(buf + cstr_len(buf), "%s", longmsg);
  writen(fd, buf, cstr_len(buf));
}

void serve_static(
  int out_fd, int in_fd, http_request* req, size_t total_size) {
  char buf[256];
  if (req->offset > 0) {
    sprintf(buf, "HTTP/1.1 206 Partial\r\n");
    sprintf(buf + cstr_len(buf), "Content-Range: bytes %lu-%lu/%lu\r\n",
      req->offset, req->end, total_size);
  } else {
    sprintf(buf, "HTTP/1.1 200 OK\r\nAccept-Ranges: bytes\r\n");
  }
  sprintf(buf + cstr_len(buf), "Cache-Control: no-cache\r\n");
  // sprintf(buf + cstr_len(buf), "Cache-Control: public,
  // max-age=315360000\r\nExpires: Thu, 31 Dec 2037 23:55:55 GMT\r\n");

  sprintf(
    buf + cstr_len(buf), "Content-length: %lu\r\n", req->end - req->offset);
  sprintf(buf + cstr_len(buf), "Content-type: %s\r\n\r\n",
...
int main(int argc, char** argv) {
  ...
  int default_port = 8182, listenfd, connfd;
  char buf[256];
