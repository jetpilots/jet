...
typedef struct {
  ...
  // example, with 7 chunks, we have 3 entries in the stack. Adding an 8th
  // chunk requires a 4th entry, rather than merging everything down to 1,
  // because we don't know whether more input is coming. This is different
  // from how the reference implementation does things.
...
static const uint32_t IV[8] = { 0x6A09E667UL, 0xBB67AE85UL, 0x3C6EF372UL,
  0xA54FF53AUL, 0x510E527FUL, 0x9B05688CUL, 0x1F83D9ABUL, 0x5BE0CD19UL };
...
// Largest power of two less than or equal to x. As a special case, returns
// 1 when x is 0.
...
// interface) are represented as words. This avoids unnecessary
// bytes<->words conversion overhead in the portable implementation.
// However, the hash_many interface handles both user input and parent node
// blocks, so it accepts bytes. For that reason, chaining values in the CV
...
// Use SIMD parallelism to hash up to MAX_SIMD_DEGREE chunks at the same
// time on a single thread. Write out the chunk chaining values and return
...
// Use SIMD parallelism to hash up to MAX_SIMD_DEGREE parents at the same
// time on a single thread. Write out the parent chaining values and return
// the number of parents hashed. (If there's an odd input chaining value
...
// and returns the length of that array. The number of chaining values
// returned is the dyanmically detected SIMD degree, at most
// MAX_SIMD_DEGREE. Or fewer, if the input is shorter than that many chunks.
// The reason for maintaining a wide array of chaining values going back up
// the tree, is to allow the implementation to hash as many parents in
// parallel as possible.
//
// As a special case when the SIMD degree is 1, this function will still
// return at least 2 outputs. This guarantees that this function doesn't
// perform the root compression. (If it did, it would use the wrong flags,
// and also we wouldn't be able to implement exendable ouput.) Note that
// this function is not used when the whole input is only 1 chunk long;
// that's a different codepath.
//
// Why not just have the caller split the input on the first update(),
...
static size_t blake3_compress_subtree_wide(const uint8_t* input,
  ...
  // only optimal as long as the SIMD degree is a power of 2. If we ever get
  // a SIMD degree of 3 or something, we'll need a more complicated
  // strategy.)
...
// resulting list of chaining values down to a single parent node. Don't
// compress that last parent node, however. Instead, return its message
// bytes (the concatenated chaining values of its children). This is
// necessary when the first call to update() supplies a complete subtree,
// because the topmost parent node of that subtree could end up being the
// root. It's also necessary for extended output in the general case.
//
...
// different from the reference implementation. Another difference is that
// we aren't always merging 1 chunk at a time. Instead, each CV might
// represent any power-of-two number of chunks, as long as the
// smaller-above-larger stack order is maintained. Instead of the "count the
// trailing 0-bits" algorithm described in the spec, we use a "count the
// total number of 1-bits" variant that doesn't require us to retain the
// subtree size of the CV on top of the stack. The principle is the same:
// each CV that should remain in the stack is represented by a 1-bit in the
// total number of chunks (or bytes) so far.
...
// chunk_state. If the user gives us 64 KiB, we want to parallelize over all
// 64 KiB at once as a single subtree, if at all possible.
//
// This leads to two problems:
// 1) This 64 KiB input might be the only call that ever gets made to
// update.
//    In this case, the root node of the 64 KiB subtree would be the root
//    node of the whole tree, and it would need to be ROOT finalized. We
//    can't compress it until we know.
...
//    compress the node at the root of the 256 KiB subtree until we know how
//    to finalize it.
//
// The second problem is solved with "lazy merging". That is, when we're
// about to add a CV to the stack, we don't merge it with anything first, as
// the reference impl does. Instead we do merges using the *previous* CV
...
void blake3_hasher_update(
  ...
  // than a single chunk (so, definitely not the root chunk), hash the
  // largest whole subtree we can, with the full benefits of SIMD (and maybe
  // in the future, multi-threading) parallelism. Two restrictions:
  ...
  //   waiting for 1 more chunk, we can't hash a subtree of 4 chunks. We
  //   have to complete the current subtree first.
  ...
  while (input_len > BLAKE3_CHUNK_LEN) {
    ...
    // remain odd, and we'll never graduate to higher degrees of
    // parallelism. See https://github.com/BLAKE3-team/BLAKE3/issues/69.
...
void blake3_hasher_finalize_seek(const blake3_hasher* self, uint64_t seek,
  ...
  // roll-up merge between that chunk hash and every subtree in the stack.
  // In this case, the extra merge loop at the end of blake3_hasher_update
  // guarantees that none of the subtrees in the stack need to be merged
  // with each other first. Otherwise, if there are no bytes in the chunk
  // state, then the top of the stack is a chunk hash, and we start the
  // merge from that.
  output_t output;
...
#endif
#if !defined(BLAKE3_NO_AVX2)
  if (features & AVX2) { return 8; }
#endif
#if !defined(BLAKE3_NO_SSE41)
  if (features & SSE41) { return 4; }
#endif
#if !defined(BLAKE3_NO_SSE2)
  if (features & SSE2) { return 4; }
#endif
#endif
#if defined(BLAKE3_USE_NEON)
  return 4;
