...
JEMALLOC_ALWAYS_INLINE bool malloc_init(void) {
    if (unlikely(!malloc_initialized()) && malloc_init_hard()) { return true; }
    return false;
}
...
#define CONF_HANDLE_CHAR_P(o, n, d)                                            \
    if (CONF_MATCH(n)) {                                                       \
        ...
        CONF_CONTINUE;                                                         \
            ...
            CONF_HANDLE_SSIZE_T(opt_lg_tcache_max, "lg_tcache_max", -1,
                (sizeof(size_t) << 3) - 1)
...
#define UNLOCK_RETURN(tsdn, ret, reentrancy)                                   \
    ...
    if (malloc_init_hard_finish()) { UNLOCK_RETURN(tsd_tsdn(tsd), true, true) }
    post_reentrancy(tsd);
...
JEMALLOC_ALWAYS_INLINE void* imalloc_no_sample(static_opts_t* sopts,
    dynamic_opts_t* dopts, tsd_t* tsd, size_t size, size_t usize, szind_t ind) {
    tcache_t* tcache;
    arena_t* arena;
...
JEMALLOC_ALWAYS_INLINE bool compute_size_with_overflow(
    ...
    *size = dopts->item_size * dopts->num_items;
    ...
    if (likely(*size / dopts->item_size == dopts->num_items)) { return false; }
    return true;
}
...
static void* irealloc_prof_sample(tsd_t* tsd, void* old_ptr, size_t old_usize,
    size_t usize, prof_tctx_t* tctx, hook_ralloc_args_t* hook_args) {
    void* p;
...
static void* irallocx_prof_sample(tsdn_t* tsdn, void* old_ptr, size_t old_usize,
...
static size_t ixallocx_prof_sample(tsdn_t* tsdn, void* ptr, size_t old_usize,
    size_t size, size_t extra, size_t alignment, bool zero, prof_tctx_t* tctx) {
    size_t usize;
...
extent_t* arena_extent_alloc_large(
    ...
    if (extent == NULL && arena_may_have_muzzy(arena)) {
        extent = extents_alloc(tsdn, arena, &extent_hooks,
            &arena->extents_muzzy, NULL, usize, sz_large_pad, alignment, false,
            szind, zero, &commit);
    }
    ...
    for (extent_t* extent = extent_list_first(decay_extents); extent != NULL;
        ...
        case extent_state_dirty:
            if (!all && muzzy_decay_ms != 0
                && !extent_purge_lazy_wrapper(tsdn, arena, r_extent_hooks,
...
static bool arena_decay_impl(tsdn_t* tsdn, arena_t* arena, arena_decay_t* decay,
...
static extent_t* arena_slab_alloc(tsdn_t* tsdn, arena_t* arena, szind_t binind,
    ...
    if (slab == NULL) {
        slab = arena_slab_alloc_hard(
            tsdn, arena, &extent_hooks, bin_info, szind);
        if (slab == NULL) { return NULL; }
    }
...
void arena_tcache_fill_small(tsdn_t* tsdn, arena_t* arena, tcache_t* tcache,
    ...
    for (i = 0,
        nfill
         = (tcache_bin_info[binind].ncached_max >> tcache->lg_fill_div[binind]);
         i < nfill; i += cnt) {
        extent_t* slab;
        if ((slab = bin->slabcur) != NULL && extent_nfree_get(slab) > 0) {
            unsigned tofill = nfill - i;
            cnt = tofill < extent_nfree_get(slab) ? tofill
                                                  : extent_nfree_get(slab);
            arena_slab_reg_alloc_batch(
                slab, &bin_infos[binind], cnt, tbin->avail - nfill + i);
        } else {
            cnt = 1;
            void* ptr
                = arena_bin_malloc_hard(tsdn, arena, bin, binind, binshard);
            /*
...
static void background_thread0_work(tsd_t* tsd) {
    ...
    unsigned i;
    for (i = 1; i < max_background_threads; i++) { created_threads[i] = false; }
...
void background_thread_interval_check(
    ...
    if (nstime_compare(&diff, &decay->epoch) <= 0) { goto label_done_unlock2; }
...
static void base_unmap(tsdn_t* tsdn, extent_hooks_t* extent_hooks, unsigned ind,
    ...
    while (block != NULL) {
        ...
        if (config_stats) {
            base->n_thp += HUGEPAGE_CEILING(
                               block->size - extent_bsize_get(&block->extent))
                >> LG_HUGEPAGE;
        }
        block = block->next;
...
static void base_extent_bump_alloc_post(
...
#undef OP
        ...
        if (!destroyed) {
            ...
            assert(
                atomic_load_zu(&astats->astats.allocated_large, ATOMIC_RELAXED)
                == 0);
        }
...
static unsigned ctl_arena_init(tsd_t* tsd, extent_hooks_t* extent_hooks) {
    ...
    if (arenas_i_impl(tsd, arena_ind, false, true) == NULL) { return UINT_MAX; }
...
#define READ_GLOBAL_MUTEX_PROF_DATA(i, mtx)                                    \
    ...
    malloc_mutex_unlock(tsdn, &mtx);
        ...
        if (have_background_thread) {
            ...
            memset(&ctl_stats
                        ->mutex_prof_data[global_prof_mutex_background_thread],
...
static int ctl_lookup(tsdn_t* tsdn, const char* name, ctl_node_t const** nodesp,
...
static int epoch_ctl(tsd_t* tsd, const size_t* mib, size_t miblen, void* oldp,
    size_t* oldlenp, void* newp, size_t newlen) {
    int ret;
...
static int background_thread_ctl(tsd_t* tsd, const size_t* mib, size_t miblen,
    void* oldp, size_t* oldlenp, void* newp, size_t newlen) {
    int ret;
    bool oldval;
...
static int thread_tcache_flush_ctl(tsd_t* tsd, const size_t* mib, size_t miblen,
    void* oldp, size_t* oldlenp, void* newp, size_t newlen) {
    int ret;
...
static int thread_prof_name_ctl(tsd_t* tsd, const size_t* mib, size_t miblen,
    void* oldp, size_t* oldlenp, void* newp, size_t newlen) {
    int ret;
...
static int thread_prof_active_ctl(tsd_t* tsd, const size_t* mib, size_t miblen,
    void* oldp, size_t* oldlenp, void* newp, size_t newlen) {
    int ret;
    bool oldval;
...
static int arena_i_initialized_ctl(tsd_t* tsd, const size_t* mib, size_t miblen,
    void* oldp, size_t* oldlenp, void* newp, size_t newlen) {
    int ret;
    tsdn_t* tsdn = tsd_tsdn(tsd);
    unsigned arena_ind;
...
static int arena_i_decay_ms_ctl_impl(tsd_t* tsd, const size_t* mib,
    ...
    if (newp != NULL) {
        ...
        if (dirty ? arena_dirty_decay_ms_set(
                tsd_tsdn(tsd), arena, *(ssize_t*)newp)
    ...
    if (arena_ind < narenas_total_get()) {
        ...
        if (arena == NULL) {
            ...
            if (newp != NULL) {
                ...
                WRITE(new_extent_hooks, extent_hooks_t*);
                old_extent_hooks
                    = extent_hooks_set(tsd, arena, new_extent_hooks);
                READ(old_extent_hooks, extent_hooks_t*);
            } else {
...
CTL_RO_CGEN(config_stats, stats_arenas_i_large_nrequests,
    ctl_arena_stats_read_u64(&arenas_i(mib[2])->astats->astats.nrequests_large),
    uint64_t)
/*
...
CTL_RO_CGEN(config_stats, stats_arenas_i_large_nflushes,
    ctl_arena_stats_read_u64(&arenas_i(mib[2])->astats->astats.nflushes_large),
    uint64_t)
...
static int stats_mutexes_reset_ctl(tsd_t* tsd, const size_t* mib, size_t miblen,
    void* oldp, size_t* oldlenp, void* newp, size_t newlen) {
...
CTL_RO_CGEN(config_stats, stats_arenas_i_lextents_j_nmalloc,
    ctl_arena_stats_read_u64(&arenas_i(mib[2])->astats->lstats[mib[4]].nmalloc),
    uint64_t)
CTL_RO_CGEN(config_stats, stats_arenas_i_lextents_j_ndalloc,
    ctl_arena_stats_read_u64(&arenas_i(mib[2])->astats->lstats[mib[4]].ndalloc),
    uint64_t)
...
static void extents_abandon_vm(tsdn_t* tsdn, arena_t* arena,
    ...
    if (config_stats) { arena_stats_accum_zu(&arena->stats.abandoned_vm, sz); }
    /*
...
static extent_t* extent_recycle_split(tsdn_t* tsdn, arena_t* arena,
    ...
    extent_split_interior_result_t result
        = extent_split_interior(tsdn, arena, r_extent_hooks, rtree_ctx, &extent,
            &lead, &trail, &to_leak, &to_salvage, new_addr, size, pad,
            alignment, slab, szind, growing_retained);
    ...
    if (result == extent_split_interior_ok) {
        ...
        if (to_leak != NULL) {
            ...
            extents_abandon_vm(tsdn, arena, r_extent_hooks, extents, to_leak,
                growing_retained);
...
static extent_t* extent_try_coalesce_impl(tsdn_t* tsdn, arena_t* arena,
    ...
    do {
        ...
        if (next != NULL) {
            ...
            if (can_coalesce
                && !extent_coalesce(tsdn, arena, r_extent_hooks, extents,
                    extent, next, true, growing_retained)) {
                if (extents->delay_coalesce) {
        ...
        if (prev != NULL) {
            ...
            if (can_coalesce
                && !extent_coalesce(tsdn, arena, r_extent_hooks, extents,
                    extent, prev, false, growing_retained)) {
                extent = prev;
                if (extents->delay_coalesce) {
                    /* Do minimal coalescing. */
                    *coalesced = true;
    ...
    bool err = ((*r_extent_hooks)->commit == NULL
        || (*r_extent_hooks)
               ->commit(*r_extent_hooks, extent_base_get(extent),
                   extent_size_get(extent), offset, length,
                   arena_ind_get(arena)));
    if (*r_extent_hooks != &extent_hooks_default) {
        extent_hook_post_reentrancy(tsdn);
    }
...
static bool extent_merge_impl(tsdn_t* tsdn, arena_t* arena,
    ...
    if (*r_extent_hooks == &extent_hooks_default) {
        ...
        extent_hook_pre_reentrancy(tsdn, arena);
        err = (*r_extent_hooks)
                  ->merge(*r_extent_hooks, extent_base_get(a),
                      extent_size_get(a), extent_base_get(b),
...
void extent_util_stats_verbose_get(tsdn_t* tsdn, const void* ptr, size_t* nfree,
    size_t* nregs, size_t* size, size_t* bin_nfree, size_t* bin_nregs,
    void** slabcur_addr) {
    assert(ptr != NULL && nfree != NULL && nregs != NULL && size != NULL
        ...
        while (true) {
            ...
            if (dss_prev == max_cur) {
                ...
                if (*zero && *commit) {
                    ...
                    extent_init(&extent, arena, ret, size, size, false,
                        SC_NSIZES, extent_state_active, false, true, true,
                        EXTENT_NOT_HEAD);
...
void hook_invoke_alloc(hook_alloc_t type, void* result, uintptr_t result_raw,
    ...
    if (h != NULL) { h(hook.hooks.extra, type, result, result_raw, args_raw); }
    FOR_EACH_HOOK_END
...
static bool large_ralloc_no_move_expand(
    ...
    if (zero) {
        if (config_cache_oblivious) {
             ...
             */
            void* zbase
                = (void*)((uintptr_t)extent_addr_get(extent) + oldusize);
...
void nstime_imultiply(nstime_t* time, uint64_t multiplier) {
    assert((((time->ns | multiplier) & (UINT64_MAX << (sizeof(uint64_t) << 2)))
               == 0)
...
static prof_gctx_t* prof_gctx_create(tsdn_t* tsdn, prof_bt_t* bt) {
    ...
    prof_gctx_t* gctx = (prof_gctx_t*)iallocztm(tsdn, size, sz_size2index(size),
        false, NULL, true, arena_get(TSDN_NULL, 0, true), true);
...
static bool prof_dump_gctx(tsdn_t* tsdn, bool propagate_err, prof_gctx_t* gctx,
...
static bool prof_dump_file(tsd_t* tsd, bool propagate_err, const char* filename,
...
bool prof_log_rep_check(void) {
    ...
    if (log_alloc_last != NULL && log_alloc_last->next != NULL) { return true; }
...
int prof_thread_name_set(tsd_t* tsd, const char* thread_name) {
    ...
    if (strlen(s) > 0) { tdata->thread_name = s; }
    return 0;
}
...
static void mutex_stats_init_cols(emitter_row_t* row, const char* table_name,
    emitter_col_t* name,
...
static void mutex_stats_read_arena(unsigned arena_ind,
    mutex_prof_arena_ind_t mutex_ind, const char* name, emitter_col_t* col_name,
    emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters],
    emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters],
    uint64_t uptime) {
    char cmd[MUTEX_CTL_STR_MAX_LENGTH];
...
static void stats_arena_bins_print(
    ...
    for (j = 0, in_gap = false; j < nbins; j++) {
        ...
        CTL_M2_M4_GET("stats.arenas.0.bins.0.nonfull_slabs", i, j,
            &nonfull_slabs, size_t);
...
static void stats_print_helper(emitter_t* emitter, bool merged, bool destroyed,
...
static void size_class(
    ...
    sc->ndelta = ndelta;
    sc->psz = (reg_size_compute(lg_base, lg_delta, ndelta) % (ZU(1) << lg_page)
        == 0);
...
void tcache_bin_flush_small(tsd_t* tsd, tcache_t* tcache, cache_bin_t* tbin,
    ...
    while (nflush > 0) {
        ...
        if (config_prof && bin_arena == arena) {
            if (arena_prof_accum(
                    tsd_tsdn(tsd), arena, tcache->prof_accumbytes)) {
