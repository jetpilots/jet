...
/*
 * The following hash function is based on MurmurHash3, placed into the
 * public domain by Austin Appleby.  See
 * https://github.com/aappleby/smhasher for details.
 */
...
static inline uint32_t hash_x86_32(
...
static inline void hash_x86_128(
  const void* key, const int len, uint32_t seed, uint64_t r_out[2]) {
  const uint8_t* data = (const uint8_t*)key;
...
static inline void hash_x64_128(
...
static inline void hash(
  const void* key, size_t len, const uint32_t seed, size_t r_hash[2]) {
