...
void resolve(Var& var) {
    ...
    if (not *var.typeInfo.name) return;
    TypeTypes tyty = TypeType_byName(var.typeInfo.name);
    if (tyty) {
        typeInfo.typeType = tyty;
    } else {
        Type* ty = type(var.typeInfo.name);
        if (type) {
            var.typeInfo.typeType = TYObject;
            var.typeInfo.type = ty;
            return;
        }
        errs.unknownType(var.typeInfo);
        return;
    }
...
void resolve(Expr& expr, Expr* parent, Scope& scope) { // if (!expr) return;
    ...
    case tkIdentifierResolved:
        break;
    case tkIdentifier:
    // TODO check parent here if tkFuncCall. Then you don't need inFuncCall
    // arg
    case tkSubscript: {
        TokenKind ret = (expr.kind == tkIdentifier) ? tkIdentifierResolved
                                                    : tkSubscriptResolved;
        ASTVar* found = lookupVar(scope, expr.string);
        if (found) {
            expr.kind = ret;
            expr.var = found;
            expr.var->used = true;
        } else {
            Import* import = lookupImport(expr.string);
            if (import) {
        ...
        if (expr.is(tkSubscriptResolved) or expr.is(tkSubscript))
            resolve(expr.left, expr, scope, inFuncCall);
    } break;
    case tkFunctionCall:
        if (expr.left) resolve(expr.left, expr, scope, true);
        break;
    case tkPeriod:
        if (expr.left) resolve(expr.left, expr, scope, inFuncCall);
        ...
        if (expr.right->is(tkSubscript) or expr.right->is(tkSubscriptResolved))
            resolve(expr.right->left, expr.right, scope, inFuncCall);
        break;
    case tkString: {
    ...
    default:
        if (expr.prec) {
            ...
            resolve(expr.right, expr, scope, inFuncCall);
            ...
            if (expr.isSelfMutOp()) {
                ...
                if (var) {
                    ...
                    if (!var->mutable) errs.immutableVar(target);
                }
            }
        }
    }
...
void resolve(Expr& expr, Expr* parent, Type& type) {
    ...
    TokenKind ret = (expr.kind == tkIdentifier) ? tkIdentifierResolved
                                                : tkSubscriptResolved;
    Var* found = nullptr;
    if (type.body) found = type.body.var(expr.string);
    if (found) {
        expr.kind = ret;
        expr.var = found;
        expr.var.used = true;
