...
void ASTScope::lowerElementalOps() {
    for (auto& stmt : this->stmts) {

        if (stmt.isCtrlExpr() && stmt.body) stmt.body->lowerElementalOps();
...
void ASTScope::promoteCandidates() {
    int tmpCount = 0;
    ASTExpr* pc = NULL;
    List<ASTExpr*>* prev = NULL;
    for (auto stmts = this->stmts.begin(); stmts != this->stmts.end();
         ++stmts) {
        ASTExpr& stmt = *stmts;

        if (stmt.isCtrlExpr() && stmt.body) stmt.body->promoteCandidates();

    startloop:

        if (!(pc = stmt.findPromotionCandidate())) {
            prev = stmts;
            continue;
        }
        if (pc == &stmt) {
...
void ASTScope::emit(int level) {
    ...
    for (auto& stmt : this->stmts) {
        ...
        if (stmt.kind != tkVarAssign || stmt.var->used) {
            if (genCoverage || genLineProfile) printf("    /************/ ");
            if (genCoverage) printf("JET_COVERAGE_UP(%d); ", stmt.line);
            if (genLineProfile) { printf("JET_PROFILE_LINE(%d);", stmt.line); }
...
void ASTScope::lint(int level) {
    for (auto exprList = this->stmts.begin(); exprList != this->stmts.end();
         ++exprList) {
        ASTExpr& expr = *exprList;
...
ASTVar* ASTScope::var(const char* name) {
    for (auto& local : this->locals)
        if (local ^= name) return &local;
    if (this->parent) return this->parent->var(name);
    return NULL;
}
