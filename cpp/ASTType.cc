...
void ASTType::emit(int level) {
    ...
    for (auto& stmt : this->body->stmts) {
        if (!stmt || stmt.kind != tkVarAssign || !stmt.var->init) continue;
...
void ASTType::genhEnum(int level) {
    if (!this->body || !this->analysed) return;
    const char* const name = this->name;
    puts("typedef enum {");

    for (auto& var : this->body->locals) printf("    %s_%s,\n", name, var.name);
    printf("} %s;\n", name);
    ...
    for (auto& var : this->body->locals) printf("    \".%s\",\n", var.name);
    puts("};");
...
void ASTType::genJson() {
    printf(
        "  void %s_json_(const %s this, int nspc) {\n", this->name, this->name);
...
void ASTType::genJsonReader() { }

void ASTType::lint(int level) {
    if (this->isEnum)
        lintEnum(level);
    else
        lintType(level);
...
void ASTType::lintType(int level) {
...
void ASTType::lintEnum(int level) {
...
const char* ASTType::getDefaultValueForType(ASTTypeSpec* type) {
    if (!type) return "";
...
ASTVar* ASTType::var(const char* name) {
