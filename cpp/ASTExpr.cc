...
private:
    ...
    bool isLiteralExpr() { return false; }
    bool isComparatorExpr() { return false; }

    bool isCmpOp() {
        return this->kind == tkOpLE || this->kind == tkOpLT
            || this->kind == tkOpGT || this->kind == tkOpGE
            || this->kind == tkOpEQ || this->kind == tkOpNE;
    }
    ...
    void emit_tkSubscriptResolved(int level) {
        ...
        case tkNumber:
            printf("Array_get_%s(%s, %s)",
                this->var->typeSpec->ASTTypeSpec_cname(), name, index->string);
            break;
        ...
        case tkOpComma:
            ...
            printf("Tensor%dD_get_%s(%s, {", this->var->typeSpec->dims,
                this->var->typeSpec->ASTTypeSpec_cname(), name);
            index.emit(0);
            printf("})");
        ...
        case tkOpColon:
            ...
            printf("Array_getSlice_%s(%s, ",
                this->var->typeSpec->ASTTypeSpec_name(), name);
            index.emit(0);
            printf(")");
            break;
    ...
    void emit(int level) {
        ...
        case tkOpLT:
            ...
            bool leftBr = this->left && this->left->prec
                && this->left->prec < this->prec;
    ...
    ASTExpr* findPromotionCandidate() {
        ...
        default:
            if (this->prec) {
                if (this->right
                    && (ret = this->right->findPromotionCandidate()))
                    return ret;
                if (!this->unary)
                    if ((ret = this->left->findPromotionCandidate()))
                        return ret;
            }
        }
        return NULL;
    ...
    void lint(int level, bool spacing, bool escapeStrings) {
        ...
        case tkLineComment:
            printf(
                "%s%s", tkLineComment.repr(*this->string != ' '), this->string);
            break;
        ...
        case tkBraceOpen:
            ...
            if (this->right)
                this->right.lint(
                    level, this->kind != tkArrayOpen, escapeStrings);
        ...
        default:
            ...
            if (this->left)
                this->left.lint(0,
                    spacing && !leftBr && this->kind != tkOpColon,
                    escapeStrings);
            if (leftBr) putc(lpc, stdout);
            ...
            if (this->right)
                this->right.lint(0,
                    spacing && !rightBr && this->kind != tkOpColon,
                    escapeStrings);
            if (rightBr) putc(rpc, stdout);
    ...
    static ASTTypeSpec* getObjectTypeSpec() {
        if (!this) return NULL;
