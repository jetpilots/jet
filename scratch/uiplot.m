...
static double sx[] = { 100, 200, 300, 400, 450 },
              sy[] = { 40, 314, 357, 127, 244 }, sr[] = { 10, 10, 20, 10, 10 };
static NSPoint spoints[] = { { 0, 65 }, { 100, 313 }, { 200, 166 },
    { 300, 122 }, { 400, 255 }, { 500, 285 } };
//-------------------------------------------------------------------------
...
@private
    ...
    int count;
    struct {
        double xmin, xmax, ymin, ymax, xspan, yspan;
    } bounds;
}
@end
//-------------------------------------------------------------------------
...
@private
    ...
    int count;
    struct {
        double xmin, xmax, ymin, ymax, rmin, rmax, rspan, xspan, yspan;
    } bounds;
...
- (void)mouseDown:(NSEvent*)event {
    ...
    printf("%g %g\n",
        (loc.x - marginleft) / plotSize.width * bounds.xspan + bounds.xmin,
        (loc.y - marginbottom) / plotSize.height * bounds.yspan + bounds.ymin);
}
...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    plotSize = NSMakeSize(dirtyRect.size.width - marginleft - marginright,
        dirtyRect.size.height - margintop - marginbottom);
    ...
    for (int i = 0; i <= ngridx; i++) {
        ...
        NSString* label = [[NSString alloc]
            initWithFormat:@"%g", bounds.xmin + i * bounds.xspan / ngridx];
        NSSize labelsize = [label sizeWithAttributes:fontInfo];
        [label drawAtPoint:NSMakePoint(point1.x - labelsize.width / 2,
                               point1.y - labelsize.height * 1.5)
            withAttributes:fontInfo];
    }
    ...
    for (int i = 0; i <= ngridy; i++) {
        NSPoint point2
            = { marginleft, marginbottom + i * plotSize.height / ngridy };
        NSString* label = [[NSString alloc]
            initWithFormat:@"%g", bounds.ymin + i * bounds.yspan / ngridy];
        NSSize labelsize = [label sizeWithAttributes:fontInfo];
        [label drawAtPoint:NSMakePoint(point2.x - labelsize.width
                                   - labelsize.height / 2,
                               point2.y - labelsize.height / 2)
            withAttributes:fontInfo];
    }

    NSAffineTransform* xform = [NSAffineTransform transform];
    [xform scaleXBy:scalex yBy:scaley];
    [xform translateXBy:-bounds.xmin + (marginleft) / scalex
                    yBy:-bounds.ymin + marginbottom / scaley];
...
- (void)computeBounds {
    ...
    // TODO: apply some scaling to r[i]s so the circle size distribution can be
    // easy on the eyes
}
@end
...
NSMenu* makeMainMenu() {
    ...
    id mainAppMenuItem = [[NSMenuItem alloc]
        initWithTitle:@"Application"
               action:nil
        keyEquivalent:@""]; // `title` really doesn't matter.
    id mainFileMenuItem = [[NSMenuItem alloc] initWithTitle:@"File"
                                                     action:nil
                                              keyEquivalent:@""];
    ...
    id mHideOthers =
        [[NSMenuItem alloc] initWithTitle:@"Hide Others"
                            ...
                            keyEquivalent:@"h"];
    [mHideOthers setKeyEquivalentModifierMask:NSEventModifierFlagOption |
    ...
    [appMenu addItem:[NSMenuItem separatorItem]];
    [[appMenu addItemWithTitle:@"Services" action:nil
                 keyEquivalent:@""] setSubmenu:appServicesMenu];
    [appMenu addItem:[NSMenuItem separatorItem]];
...
int main(int argc, char* argv[]) {
    // #ifdef CLI_MODE
    //         @autoreleasepool {
    //                 MyCLIController* con = [[[MyCLIController alloc]
    //                 initWithArgs:argv count:argc] autorelease];
    //         }
    //         return [con run];
    // #else
    @autoreleasepool {
    ...
    // NSWindow* window = [[[NSWindow alloc] initWithContentRect:NSMakeRect(0,
    // 0, 200, 200)
    //                                                                 styleMask:
    ...
    // [window makeKeyAndOrderFront:nil];
    // id nsv = [[KaleidoView alloc]
    // initWithTrackingInFrame:NSMakeRect(0,0,200,200)]; [nsv
