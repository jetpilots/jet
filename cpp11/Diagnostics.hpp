...
struct Diagnostics {
    ...
    void unknownVar(Expr& expr) {
        error(ErrKind::unknown, ErrItem::var, expr.loc, &expr);
    }
    void unknownMember(Expr& expr, Type& type) {
        error(ErrKind::unknown, ErrItem::var, expr.loc, &expr);
    }
    void unknownType(Var& var) {
        error(ErrKind::unknown, ErrItem::type, var.loc, &var);
    }
    void unknownFunc(Expr& expr) {
        error(ErrKind::unknown, ErrItem::func, expr.loc, &expr);
    }
    ...
    void stringInterp(Expr& expr, const char* message = "") { }
    void unexpectedExpr(Expr& expr, const char* message = "") {
        error(ErrKind::unexpected, ErrItem::expr, {}, &expr);
    }
    void expectedToken(TokenKind kind) { }
    void tooManyDims(Var& var) {};
    void nestedType(Type& type) {};
    void nestedFunc(Func& func) {};
    void duplicateVar(Var& var, SourceLoc orig) {};
    void duplicateFunc(Func& func, SourceLoc orig) {};
    void duplicateType(Type& type, SourceLoc orig) {};
    void duplicateEnum(Type& type, SourceLoc orig) {};
    void invalidEnumMember(Expr& expr);
};