...
struct Parser {
    ...
    if (ignore(tkOneSpace) and token.is(tkIdentifier)) {
      ...
      if (matches(tkArrayDims)) {
        if (isalpha(*token.pos)) {
          ...
          var.spec.collectionType = (dims == 1) ? CTYArray : CTYTensor;
          var.spec.dims = dims;
        }
        token.advance();
      }
      ...
      case tkKeyword_while:
        ...
        if (tt == tkKeyword_for) {
          ...
          if (not expr->left)
            ...
            fvar = new Var(
                expr->left->left->string, expr->loc, expr->left->right);
            fvar->isMutable = true;
        ...
        expr->body
            = parseScope(tt == tkKeyword_for ? forScope : scope, ownerFunc);
        ...
        if (token.in(tkKeyword_else, tkKeyword_elif)) {
          ...
          discard(tkKeyword_end);
          ignore(tkOneSpace);
          ignore(tt == tkKeyword_else or tt == tkKeyword_elif ? tkKeyword_if
                                                              : tt);
        }
        scope->stmts.push(*expr);
        break;
  ...
  Expr* parseExpr(Scope& scope) {
    ...
    while (not(token.in(tkNullChar, tkNewline, tkLineComment))) {
      ...
      case tkArrayOpen:
        ops.push(expr);
        if (not ops.empty() and ops.top()->is(tkSubscript)) rpn.push(expr);
        if (lookAhead == ']') rpn.push(nullptr); // empty arr[]
        break;

      case tkBraceOpen:
        ops.push(expr);
        if (not ops.empty() and ops.top()->is(tkObjectInit)) rpn.push(expr);
      ...
      default:
        if (prec) {
          ...
          while (not ops.empty()) {
            ...
            if (not(p->in(
                    tkOpComma, tkOpSemiColon, tkFunctionCall, tkSubscript))
            ...
            if (not(p->prec or p->unary)
                and not p->in(tkFunctionCall, tkOpColon, tkSubscript)
                and rpn.count() < 2) {
  ...
  exitloop:
    ...
    while (not ops.empty()) {
      ...
      if (not(p->prec or p->unary)
          and not(p->in(tkFunctionCall, tkSubscript)) and rpn.count() < 2) {
