...
private:
    ...
    void analyse(
        ...
        case tkFunctionCall: {
                ...
                if (*buf != '<') // not invalid type
                {
                    ...
                    for (Func& func : mod.funcs) {
                        ...
                        if (!func.intrinsic
                            ...
                            and leven(expr.string, func.name, expr.loc.len,
                                    func.loc.len)
                                < 3
                            and func.args.count()
                                == expr.left->countCommaList()) {
            ...
            // -------------------------------------------------- //
        case tkVarAssign: {
            // TypeInfo& typeInfo = expr.var->typeInfo;
            Var& var = *expr.var;

            if (var.typeInfo.is(TYUnresolved)) resolve(expr.var, mod);
            ...
            if (not expr.var->init) {
                ...
                if (!var.typeInfo.dims) // goto errorMissingInit;
                    switch (var.typeInfo.typeType) {
                ...
                // if (typeInfo.is(TYUnresolved))
                //     resolveTypeSpec(typeInfo, mod);
                // first try to set enum base if applicable.
                Expr& init = *expr.var->init;

                if (expr.var->isEnum()) setEnumBase(init, typeInfo, mod);
                ...
                if (!init.typeInfo.is(TYNilType)) {
                ...
                } else if (var.typeInfo.is(TYObject)) {
                    // if nil, and typespec given and resolved, set type info
                    // from typespec.
                    expr.typeInfo = *typeInfo; //->typeInfo.typeType;

                } else if (var.typeInfo.is(TYUnresolved)) {
                    ...
                    var.typeInfo.typeType = expr.typeInfo.typeType
                        = TYErrorType;
                ...
                if (var.typeInfo.is(TYUnresolved)) {
                    var.typeInfo.typeType = init.typeInfo.typeType;
                    if (init.typeInfo.is(TYObject)) {
                        if (init.is(tkFunctionCallResolved)) {
                            ...
                            if (init.left->var->isEnum()) {
                                ...
                                // subscript
                                var.type = e->var->type;
                                var.typeInfo.dims = e->var->typeInfo.dims;
                            }
                ...
                } else if (var.typeInfo.typeType != init.typeInfo.typeType
                ...
                if (var.typeInfo.dims == 0) {
                    var.typeInfo.collectionType = init.typeInfo.collectionType;
                    var.typeInfo.dims = init.is(CTYTensor) ? init.typeInfo.dims
                        : init.is(CTYArray)                ? 1
                                                           : 0;
                } else if (var.typeInfo.dims != 1 and init.is(CTYArray)) {
                    errs.initDimsMismatch(expr, 1);
                    expr.typeInfo.typeType = TYErrorType;
                } else if (var.typeInfo.dims != 2 and init.is(CTYTensor)) {
                    errs.initDimsMismatch(expr, 2);
                    expr.typeInfo.typeType = TYErrorType;

                } else if (var.typeInfo.dims != 0 and init.is(CTYNone)) {
        ...
        case tkNumber: expr.typeInfo.typeType = TYNumber; break;
        ...
        case tkIdentifierResolved:
            ...
            if (expr.typeInfo.is(TYObject)) analyse(mod, *expr.var->type);
            break;
            ...
            if (expr.right) {
                ...
                if (expr.typeInfo.typeType == TYObject) {
                    ...
                    case tkIdentifierResolved:
                        ...
                        break;
                    case tkFunctionCallResolved:
                        expr.type = first->func->ret->typeInfo.type;
                    ...
                    default:
                        ...
                        // expr.type = first->var->typeInfo.type;break;
                    }
                    // expr.type =
                }
            }
        ...
        case tkBraceOpen:
            if (expr.right) {
                ...
                if (expr.typeInfo.typeType == TYObject) {
                    ...
                    switch (first->kind) {
                    case tkIdentifierResolved:
                        expr.type = first->var->typeInfo.type();
                        break;
                    case tkFunctionCallResolved:
                        expr.type = first->func->ret->typeInfo.type();
                        break;
