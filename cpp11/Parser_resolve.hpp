...
void resolve(Var& var) {
  ...
  if (tyty) {
    ...
    Type* type = mod->getType(var.typeName());
    if (type) {
...
void resolve(Expr& expr, Expr* parent, Scope& scope, Module& mod) {
  switch (expr.kind) {
  ...
  case tkSubscript: {
    ...
    if (found) {
      ...
      Import* import = mod.getImport(expr.string);
      if (import) {
    ...
    if (expr.is(tkSubscriptResolved) or expr.is(tkSubscript))
      resolve(*expr.left, &expr, scope, mod);
  } break;
  case tkFunctionCall:
    if (expr.left) resolve(*expr.left, &expr, scope, mod);
    break;
  case tkPeriod:
    if (expr.left) resolve(*expr.left, &expr, scope, mod);
    ...
    if (expr.right->is(tkSubscript) or expr.right->is(tkSubscriptResolved))
      resolve(*expr.right->left, expr.right, scope, mod);
    break;
  case tkString: {
  ...
  default:
    if (expr.prec) {
      ...
      resolve(*expr.right, &expr, scope, mod);
...
void resolve(Expr& expr, Expr* parent, Type& type) {
  ...
  TokenKind ret = (expr.kind == tkIdentifier) ? tkIdentifierResolved
                                              : tkSubscriptResolved;
  Var* found = type.body->getVar(expr.string);
  if (found) {
    expr.kind = ret;
    expr.var = found;
    expr.var->used++;
