...
static void expr_hash(
  ...
  case tkNumber: {
    char* c = strpbrk(expr->str, "e.");
    if (c) {
      expr->hash = Real64_hash(atof(expr->str));
    } else {
      expr->hash = Int64_hash(atoll(expr->str));
    }
  } break;
  case tkVarDefn: expr_hash(parser, expr->var->init, cseDict); break;
  case tkFuncCallR:
  case tkFuncCall:
  case tkSubscript:
  case tkIdentR:
...
// the hashes have been generated in expr_hash (which is bottom-up, so
