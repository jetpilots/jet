...
static void JetExpr_hash(
    Parser* parser, JetExpr* expr, Dict(UInt32, Ptr) * cseDict) {
    if (!expr) {
    ...
    case tkVarAssign: JetExpr_hash(parser, expr->var->init, cseDict); break;
    ...
    case tkSubscriptResolved: {
        ...
        // JetFunc and JetVar, so the deref will get the right thing
        if (expr->left) {
            JetExpr_hash(parser, expr->left, cseDict);
    ...
    default:
        if (expr->prec) {
            ...
            str.hash[1] = 0;
            if (expr->right) {
                JetExpr_hash(parser, expr->right, cseDict);
...
// hashes have been generated in JetExpr_hash (which is bottom-up, so checking
// cannot happen inline).
static void JetExpr_checkHashes(
    Parser* parser, JetExpr* expr, Dict(UInt32, Ptr) * cseDict) {
    if (!expr) return;
    UInt32 idx = Dict_get(UInt32, Ptr)(cseDict, expr->hash);

    if (idx < Dict_end(cseDict)) {
        JetExpr* orig = Dict_val(cseDict, idx);
    ...
    switch (expr->kind) {
    case tkVarAssign:
        JetExpr_checkHashes(parser, expr->var->init, cseDict);
        break;
    case tkFunctionCallResolved:
    case tkFunctionCall:
    case tkSubscript:
    case tkSubscriptResolved:
        if (expr->left) JetExpr_checkHashes(parser, expr->left, cseDict);
        break;
    default:
        if (expr->prec) {
            if (!expr->unary && expr->left)
                JetExpr_checkHashes(parser, expr->left, cseDict);
            if (expr->right) JetExpr_checkHashes(parser, expr->right, cseDict);
        }
    }
}
...
static void JetFunc_hashExprs(Parser* parser, JetFunc* func) {
    ...
    foreach (JetExpr*, stmt, func->body->stmts) {
        JetExpr_hash(parser, stmt, cseDict);
        JetExpr_checkHashes(parser, stmt, cseDict);
    }
}
